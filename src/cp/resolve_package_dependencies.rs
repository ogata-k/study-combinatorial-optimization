...
mod tests {
    ...
    #[test]
    fn constraint_parse_whitespace_behavior() {
        // 先頭にスペースがある場合はパースエラー（デフォルト戦略とみなされ、バージョンパースで失敗する）
        assert!(" ^1.2.3".parse::<Constraint>().is_err());

        // 戦略記号の後のスペースは、数値パース時に無視されない
        assert!("^ 1.2.3".parse::<Constraint>().is_err());
    }
    ...
    fn resolve_picks_highest_version() {
        ...
        // A has 3 versions
        registry.add_package("A", v100, PackageInfo::new(vec![], 10));
        registry.add_package("A", v110, PackageInfo::new(vec![], 10));
        registry.add_package("A", v120, PackageInfo::new(vec![], 10));
        ...
        assert!(result.is_ok());
        let graph = result.unwrap();
        // Should pick 1.2.0 (newest)
        assert_eq!(graph.get("A").unwrap().version, v120);
    }

    #[test]
    fn resolve_ignores_unrelated_packages() {
        let mut registry = PackageRegistry::new();
        let v1 = Version::new(1, 0, 0);
        ...
        // Unrelated C -> D (D is missing)
        // Cが依存ツリーに含まれない限り、Dが存在しなくてもエラーになってはいけない
        registry.add_package(
            "C",
            v1,
            PackageInfo::new(vec![("D".to_string(), "^1.0.0".parse().unwrap())], 10),
    ...
    #[test]
    fn test_simulate_parallel_install_complex_graph() {
        // Root -> A, B
        // A -> C, D
        // B -> D, E
        // C -> F
        ...
        registry.add_package("G", v1, PackageInfo::new(vec![], 10));
        registry.add_package(
            "F",
            v1,
            PackageInfo::new(vec![("G".to_string(), "^1.0.0".parse().unwrap())], 10),
        );
        registry.add_package(
            "E",
            v1,
            PackageInfo::new(vec![("F".to_string(), "^1.0.0".parse().unwrap())], 10),
        );
        registry.add_package(
            "D",
            v1,
        ...
        registry.add_package(
            ...
            PackageInfo::new(
                vec![
                    ("D".to_string(), "^1.0.0".parse().unwrap()),
                    ("E".to_string(), "^1.0.0".parse().unwrap()),
                ],
                10,
            ),
        );
        ...
        println!("--- Complex Simulation ---");
        simulate_parallel_install(&registry, &graph, 4);
    }
}
